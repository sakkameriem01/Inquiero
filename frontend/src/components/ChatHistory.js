...
const ChatHistory = ({ onSelectSession, onNewChat, currentSession }) => {
  ...
  const handleViewSession = async (session) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/sessions/${Number(session.id)}`);
  ...
  const handleSaveSession = async (session) => {
    try {
      const response = await fetch(`${API_BASE_URL}/chat-sessions/${Number(session.id)}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDeleteSession = (sessionId) => {
    setConfirmModalMessage('Are you sure you want to delete this chat?');
    setConfirmAction(() => async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/chat-sessions/${Number(sessionId)}`, {
          method: 'DELETE',
        });
  ...
  const handleDeleteAllSessions = () => {
    setConfirmModalMessage('Are you sure you want to delete all the chats?');
    setConfirmAction(() => async () => {
      try {
        ...
        // Clear local state
        setSessions([]);
        onNewChat(); // Reset current session
